...
//! 10.5) =-=-=-=-=-= Update System Time Input (Browser Local Time) =-=-=-=-=-= !///
function updateSystemTimeInput() {
  const sysInput = document.getElementById("systemTime");
  if (!sysInput) return;

  const now = new Date(); // زمان سیستم کاربر (browser)
  sysInput.value = formatYMDHMS(now); // "YYYY-MM-DD HH:MM:SS"
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  setInterval(updateSystemTimeInput, 1000);
    ...
    saveManualBtn.addEventListener("click", async () => {
      try {
        ...
        if (!toggle || !toggle.checked) {
          toast.error("Manual Time Sync is disabled.");
          return;
        }

        // بعد از موفقیت API، توکن پاک می‌شود (بدون ریدایرکت مستقیم به لاگین)
        await Auth.clearTokenAfter(saveManualTimeIfEnabled(), {
  ...
  //! 19.3-bis) =-=-=-=-=-= Sync System Time Btn =-=-=-=-=-= !///
  const syncSystemBtn = document.getElementById("syncSystemTime");
  if (syncSystemBtn) {
    syncSystemBtn.addEventListener("click", async () => {
      try {
        const now = new Date();
        const payload = formatYMDHMS(now); // "YYYY-MM-DD HH:MM:SS"

        await Auth.clearTokenAfter(
          apiPost(API_ENDPOINTS.localTime, { local_time: payload }),
          { redirectToLogin: false }
        );

        toast.success("Device time synced with system time.");
        setTimeout(() => window.location.reload(), 2000);
      } catch (e) {
        console.error("Failed to sync system time:", e);
        toast.error("Syncing system time failed.");
      }
    });
  }
